...
def convertDataForJsonFile(data):
    ...
    convertedData["name"] = convertedData["name"] if convertedData["name"] is not None else ""
    return {
        "datetime_utc": convertedData["datetime_utc"],
        "distance_au": convertedData["distance_au"],
        "velocity_km_s": convertedData["velocity_km_s"],
        "neo": {
            "designation": convertedData["designation"],
            "name": convertedData["name"],
            "diameter_km": convertedData["diameter_km"],
            "potentially_hazardous": convertedData["potentially_hazardous"]
        }
    }
...
def write_to_csv(results, filename):
    ...
    with open(filename, 'w', newline='') as new_csv_file:
        ...
        for result in results:
            convertedData = convertDataForCSVFile(result)
            writer.writerow(convertedData)
...
def write_to_json(results, filename):
    ...
    with open(filename, 'w') as new_json_file:
        json_data = []
        for result in results:
            convertedData = convertDataForJsonFile(result)
            json_data.append(convertedData)
        json.dump(json_data, new_json_file, indent="\t")
